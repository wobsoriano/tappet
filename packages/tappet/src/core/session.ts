...
function unusable(state: SessionState): TappetError {
  ...
  return new TappetError({ kind: 'session-closed', command: 'run a device command' });
}
...
export function failureOf(error: unknown): DeviceFailure | null {
  if (!(error instanceof TappetError)) return null;
