...
`tappet/core` is the runner-independent half: `openSession`, `createApp`, `probe`, `TappetError`, and the types around them. It imports neither `@playwright/test` nor `agent-device`. An adapter for another runner implements `ActionSink`, calls `openSession` with its own worker slot, and registers matchers over `probe`.
