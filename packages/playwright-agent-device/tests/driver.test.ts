...
test("a session bound to another device carries what it was bound to", () => {
  const failure = classifyError(
    new AppError(
      "INVALID_ARGS",
      "open is already bound to session pwad-ios-0 on android device emulator-5554",
    ),
  );
  expect(failure.kind).toBe("session-rebound");
  if (failure.kind !== "session-rebound") return;
  expect(failure.boundTo).toBe("session pwad-ios-0 on android device emulator-5554");
});
