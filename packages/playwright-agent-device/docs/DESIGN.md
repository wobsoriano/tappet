...
src/
  ...
  core/
    index.ts               runner-independent entry: openSession, createApp, probe, DeviceTestError, types
    ...
    errors.ts              DeviceTestError, ErrorInfo
  driver/
...
1. Session name is `${prefix}-${scope}-${slot}`. `prefix` defaults to `pwad`, `scope` is the Playwright project name, `slot` is `parallelIndex`. Deterministic, so a replacement worker after a failure reuses it.
